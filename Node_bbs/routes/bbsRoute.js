...
router.get("/write", function (req, res) {
  // bbsWrite.pug파일을 rendering 하여 요청 전송
...
router.post("/write", function (req, res) {
  ...
  // req.body 객체에 b_date, b_write, b_count 변수를 생성하고
  // 각각의 변수에 값을 세팅하라
  ...
  data
    ...
    .catch(function (error) {
      // insert가 실패하면
      console.error(error); // 오류메시지를 콘솔에 보여주라
      // 웹화면에 데이터 추가가 잘못되었다는
...
// localhost:3000/bbs/view/id값의 URL 요청
// id값 : bbs의 각 라인(item)의 PK값
// PK값을 가지고 tbl_bbs에서 1 개의 item 값을 추출하여
// detail view에 보여주기
router.get("/bbs/view/:id", function (req, res) {
  ...
  bbsVO
    ...
    .then(function (result) {
      // PK 값과 일치하는 item이 있는 그 결과를 list에 담아준다.
      // res.json(result);
      // bbsView.pug를 redering 할때 bbsVO라는 이름으로 전달하여 줘라
...
조건문 없이 remove()를 실행하면 테이블의 모든데이터를 묻지도 따지지도않고
삭제 해버린다!
...
router.get("/delete/:id", function (req, res) {
  ...
  -두가지 함수를 사용할 수 있다.
  deleteOne(조건문): 삭제한후 삭제되었다는 메시지만 알고싶을때
...
/*
한개의 item을 조회하여 write form으로 보낸 후 데이터를 다시 받앗을때
req.body에는 id값이 사라진 형태로 전달된다.
